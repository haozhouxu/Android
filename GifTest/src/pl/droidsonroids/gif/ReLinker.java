...
class ReLinker {
	...
	private static File unpackLibrary(final Context context, final String libName) {
		...
		File outputFile = new File(context.getDir(LIB_DIR, Context.MODE_PRIVATE), libName + "1.1.16");
		if (outputFile.isFile()) {
			return outputFile;
		}
		//BuildConfig.VERSION_NAME
		final File cachedLibraryFile = new File(context.getCacheDir(), libName + "1.1.16");
